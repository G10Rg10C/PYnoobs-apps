...
# Crea una canvas per visualizzare l'immagine della roulette
canvas = tk.Canvas(root, width=roulette_img.width, height=roulette_img.height)
canvas.place(x=0, y=0)  # Posiziona la canvas sopra lo sfondo
...
def ruota_roulette1():
    ...
    canvas.create_image(0, 0, anchor=tk.NW, image=roulette_tk)
    canvas.image = roulette_tk
...
pulsante_ruota = tk.Button(root, text="Ruota la roulette", command=ruota_roulette)
pulsante_ruota.pack()
